    ...
    public final void act(final MapleClient c, final MapleReactor reactor) {
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("反應堆腳本", false,
                        "開啟反應堆腳本：" + reactor.getReactorId() + ".js" + c.getPlayer().getMap());
            }
            final Invocable iv = getInvocable("反應堆/" + reactor.getReactorId() + ".js", c);

            if (iv == null) {
                if (c.getPlayer().isShowErr()) {
                    c.getPlayer().showInfo("反應堆腳本", true,
                            "找不到反應堆腳本" + reactor.getReactorId() + ".js" + c.getPlayer().getMap());
                }
            ...
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6,
                        "執行反應堆腳本出錯. 反應堆ID: " + reactor.getReactorId() + ", 反應堆名稱: " + reactor.getName() + ":" + e);
            }
