...
public class EventManager {
    ...
    public void startInstance(MapleCharacter chr) {
        try {
            ...
            String err = "事件脚本: \"" + name + "\", 函數: \"setup\"，出錯：" + ex;
            if (chr != null && chr.isShowErr()) {
                chr.dropMessage(6, err);
            }
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, err);
        }
    }
    ...
    public void warpAllPlayer(int from, int to) {
        ...
        if (tomap != null && frommap != null && frommap.getCharactersSize() > 0) {
