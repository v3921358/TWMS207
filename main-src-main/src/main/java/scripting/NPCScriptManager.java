...
public class NPCScriptManager extends AbstractScriptManager {
            ...
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("NPC腳本", true,
                ...
                if (c.getPlayer().isShowErr()) {
                    c.getPlayer().showInfo("NPC腳本", true,
                            "無法執行腳本:已有腳本執行-" + cms.containsKey(c) + " | 允許執行腳本-" + c.canClickNPC());
                }
                NPCConversationManager cm = cms.get(c);
                if (cm == null || (cm.getType() != ScriptType.ON_USER_ENTER
                        && cm.getType() != ScriptType.ON_FIRST_USER_ENTER)) {
                    c.getPlayer().dropMessage(-1, "你當前已經和1個NPC對話了. 如果不是請輸入 @解卡 指令進行解卡。");
                }
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6, "執行NPC腳本出錯 : NPC - " + npc + " 特殊腳本 - " + script + " ." + e);
            }
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection) {
        if (mode != -1) {
            ...
            try {
                ...
                if (c.getPlayer().isShowErr()) {
                    c.getPlayer().dropMessage(6, str + " ." + e);
                }
                FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n" + str + " ." + e + "\r\n\r\n");
                dispose(c);
    ...
    public final void startQuest(final MapleClient c, final int npc, final int quest) {
        ...
        try {
            ...
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6, "執行任務開始腳本出錯 : NPC - " + npc + "任務 - " + MapleQuest.getInstance(quest) + " ." + e);
            }
    ...
    public final void startQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6, "執行任務開始腳本出錯 : NPC - " + cm.getNpc() + "任務 - "
                        + MapleQuest.getInstance(cm.getQuest()) + " ." + e);
            }
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n執行任務開始腳本出錯 : NPC - " + cm.getNpc() + "任務 - "
                    + MapleQuest.getInstance(cm.getQuest()) + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void endQuest(final MapleClient c, final int npc, final int quest, final boolean customEnd) {
        ...
        try {
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("任務完成腳本", true,
                                "找不到腳本任務 NPC：" + npc + " 任務：" + MapleQuest.getInstance(quest) + c.getPlayer().getMap());
                    }
                    iv = getInvocable("任務/notcoded.js", c, true); // safe disposal
                    if (iv == null) {
                        dispose(c);
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, npc, quest, null, ScriptType.QUEST_END,
                        iv);
                cms.put(c, cm);
                scriptengine.put("qm", cm);
            ...
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n執行任務完成腳本出錯 : NPC - " + npc + "任務 - "
                    + MapleQuest.getInstance(quest) + " ." + e + "\r\n\r\n");
            dispose(c);
    ...
    public final void endQuest(final MapleClient c, final byte mode, final byte type, final int selection) {
        ...
        try {
            ...
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6, "執行任務完成腳本出錯 : NPC - " + cm.getNpc() + "任務 - "
                        + MapleQuest.getInstance(cm.getQuest()) + " ." + e);
            }
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "\r\n\r\n執行任務完成腳本出錯 : NPC - " + cm.getNpc() + "任務 - "
                    + MapleQuest.getInstance(cm.getQuest()) + " ." + e + "\r\n\r\n");
            dispose(c);
            notice(c, cm.getNpc(), String.valueOf(cm.getQuest()), cm.getType());
        } finally {
    ...
    public final void startItemScript(final MapleClient c, final int itemId, final short slot, final int npc,
            final String script) {
        final Lock lock = c.getNPCLock();
        lock.lock();
        try {
            ...
            if (!cms.containsKey(c) && c.canClickNPC()) {
                Invocable iv = getInvocable("道具/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                    ...
                    System.out.println("\r\n找不到道具腳本:" + script + "\r\n");
                    iv = getInvocable("道具/notcoded.js", c, true); // safe disposal
                    if (iv == null) {
                        dispose(c);
                        return;
                    }
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new ItemScriptManager(c, npc, itemId, slot, script, ScriptType.ITEM,
                        iv);
                cms.put(c, cm);
                ...
                c.getPlayer().showInfo("道具腳本", true,
                        "無法執行腳本:已有腳本執行-" + cms.containsKey(c) + " | 允許執行腳本-" + c.canClickNPC());
            }
        } catch (final ScriptException | NoSuchMethodException e) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().dropMessage(6, "執行道具腳本出錯 : NPC - " + npc + " 腳本 - " + script + " ." + e);
            }
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log,
    ...
    public final void onUserEnter(final MapleClient c, final String script) {
        ...
        try {
            ...
            if (!cms.containsKey(c)) {
                Invocable iv = getInvocable("地圖/onUserEnter/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("onUserEnter腳本", true,
                                "找不到onUserEnter腳本:" + script + c.getPlayer().getMap());
                    }
                    System.out.println("\r\n找不到onUserEnter腳本:" + script + c.getPlayer().getMap() + "\r\n");
                    iv = getInvocable("地圖/onUserEnter/notcoded.js", c, true); // safe disposal
                    if (iv == null) {
                        dispose(c);
                        return;
                    }
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, 0, -1, script, ScriptType.ON_USER_ENTER,
                        iv);
                cms.put(c, cm);
    ...
    public final void onFirstUserEnter(final MapleClient c, final String script) {
        ...
        try {
            if (c.getPlayer().isShowInfo()) {
                c.getPlayer().showInfo("onFirstUserEnter腳本", false,
                        "開始onFirstUserEnter腳本：" + script + c.getPlayer().getMap());
            }
            if (!cms.containsKey(c)) {
                Invocable iv = getInvocable("地圖/onFirstUserEnter/" + script + ".js", c, true);
                if (iv == null) {
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("onFirstUserEnter腳本", true,
                                "找不到onFirstUserEnter腳本:" + script + c.getPlayer().getMap());
                    }
                    System.out.println("\r\n找不到onFirstUserEnter腳本:" + script + c.getPlayer().getMap() + "\r\n");
                    iv = getInvocable("地圖/onFirstUserEnter/notcoded.js", c, true); // safe disposal
                    if (iv == null) {
                        dispose(c);
                        return;
                    }
                ...
                final ScriptEngine scriptengine = (ScriptEngine) iv;
                final NPCConversationManager cm = new NPCConversationManager(c, 0, -1, script,
                        ScriptType.ON_FIRST_USER_ENTER, iv);
                cms.put(c, cm);
                scriptengine.put("ms", cm);
                c.getPlayer().setConversation(1);
                c.setClickedNPC();
        ...
        if (npccm != null) {
            ...
            switch (npccm.getType()) {
                ...
                case ITEM:
                    ...
                    c.removeScriptEngine("腳本/道具/notcoded.js");
                    break;
                case ON_USER_ENTER:
                    c.removeScriptEngine("腳本/地圖/onUserEnter/" + npccm.getScript() + ".js");
                    c.removeScriptEngine("腳本/地圖/onUserEnter/notcoded.js");
                    break;
                case ON_FIRST_USER_ENTER:
                    c.removeScriptEngine("腳本/地圖/onFirstUserEnter/" + npccm.getScript() + ".js");
                    c.removeScriptEngine("腳本/地圖/onFirstUserEnter/notcoded.js");
                    break;
                default:
                    c.removeScriptEngine("腳本/任務/" + npccm.getQuest() + ".js");
                    c.removeScriptEngine("腳本/任務/notcoded.js");
            }
        }
