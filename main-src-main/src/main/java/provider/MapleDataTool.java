...
public class MapleDataTool {
    ...
    public static int getInt(MapleData data) {
        int value;
        if (data.getData() instanceof String) {
            value = Integer.parseInt(getString(data));
        } else if (data.getData() instanceof Long) {
            value = ((Long) data.getData()).intValue();
    ...
    public static double getDoubleConvert(MapleData data) {
        if (data.getType() == MapleDataType.STRING) {
            String dd = getString(data);
            if (dd.endsWith("%")) {
                dd = dd.substring(0, dd.length() - 1);
                return Double.parseDouble(dd) / 100.0;
            }
    ...
    public static double getDoubleConvert(String path, MapleData data) {
        return getDoubleConvert(data.getChildByPath(path));
    }

    public static double getDoubleConvert(MapleData d, double def) {
        try {
            return getDoubleConvert(d);
        } catch (Exception ex) {
            return def;
        }
    }

    public static double getDoubleConvert(String path, MapleData data, double def) {
        try {
            return getDoubleConvert(data.getChildByPath(path));
        } catch (Exception ex) {
            return def;
