...
public class CheatingOffensePersister {
    ...
    public class PersistingTask implements Runnable {
        ...
        public void run() {
            ...
            /*
             * try { Connection con = DatabaseConnection.getConnection(); PreparedStatement
             * insertps = con.
             ...
             * offense.getParam() == null ? "" : offense.getParam(); if (offense.getDbId()
             * == -1) { insertps.setInt(1, offense.getChrfor()); insertps.setString(2,
             * offense.getOffense().name()); insertps.setInt(3, offense.getCount());
             * insertps.setTimestamp(4, new Timestamp(offense.getLastOffenseTime()));
             * insertps.setString(5, parm); insertps.executeUpdate(); ResultSet rs =
             * insertps.getGeneratedKeys(); if (rs.next()) { offense.setDbId(rs.getInt(1));
             * } rs.close(); } else { updateps.setInt(1, offense.getCount());
             * updateps.setTimestamp(2, new Timestamp(offense.getLastOffenseTime()));
             * updateps.setString(3, parm); updateps.setInt(4, offense.getDbId());
             * updateps.executeUpdate(); } } insertps.close(); updateps.close(); } catch
             * (SQLException e) { System.err.println("error persisting cheatlog" + e); }
             */
        }
    }
}
