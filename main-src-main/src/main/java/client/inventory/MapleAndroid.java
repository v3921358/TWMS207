...
public class MapleAndroid implements Serializable {
    ...
    public static final MapleAndroid create(int itemid, int uniqueid, StructAndroid aInfo) {
        ...
        try {
            ...
            try (PreparedStatement pse = con.prepareStatement(
                    "INSERT INTO androids (uniqueid, type, gender, skin, hair, face, name) VALUES (?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public void showEmotion(MapleCharacter chr, String emo) {
        byte emo1 = 0;
        byte emo2 = 0;
        switch (emo) {
            case "hello":
                emo1 = 0;
                emo2 = (byte) Randomizer.nextInt(2);
                break;
            case "levelup":
                emo1 = 1;
                emo2 = (byte) Randomizer.nextInt(2);
            ...
            case "alert":
                emo1 = 5;
                emo2 = (byte) (Randomizer.isSuccess(10) ? 0 : 1);
                if (emo2 == 1) {
                    return;
                }
                break;
        ...
        chr.getMap().broadcastMessage(CField.showAndroidEmotion(chr.getId(), emo1, emo2));
    }
    ...
    public static void clearAndroid() {
        try {
            ...
            PreparedStatement ps = con.prepareStatement(
                    "SELECT androids.uniqueid, androids.name, inventoryitems.uniqueid FROM inventoryitems RIGHT JOIN androids ON androids.uniqueid = inventoryitems.uniqueid where inventoryitems.uniqueid IS NULL");
            ...
            System.err.println("[MapleAndroid] 從數據庫中加載機器人訊息出錯" + se.getMessage());
        }
    }
}
