...
public class Equip extends Item implements Serializable {
    ...
    // charm: -1 = has not been initialized yet, 0 = already been worn, >0 = has teh
    // charm exp
    private byte state = 0, bonusState = 0, oldState = 0, upgradeSlots = 0, level = 0, vicioushammer = 0,
            platinumhammer = 0, enhance = 0, reqLevel = 0, yggdrasilWisdom = 0, bossDamage = 0, ignorePDR = 0,
            totalDamage = 0, allStat = 0, karmaCount = -1, flame = -1, starforce;
    private short str = 0, dex = 0, _int = 0, luk = 0, hp = 0, mp = 0, watk = 0, matk = 0, wdef = 0, mdef = 0, acc = 0,
            avoid = 0, hands = 0, speed = 0, jump = 0, charmExp = 0, pvpDamage = 0, enhanctBuff = 0, soulname,
            soulenchanter, soulpotential;
    private int durability = -1, incSkill = -1, potential1 = 0, potential2 = 0, potential3 = 0, bonuspotential1 = 0,
            bonuspotential2 = 0, bonuspotential3 = 0, fusionAnvil = 0, socket1 = 0, socket2 = 0, socket3 = 0, soulskill,
            limitBreak = 0;
    ...
    private Map<EquipStat, Integer> flameStats = new LinkedHashMap<>();
    ...
    public Equip(int id, short position, int uniqueid, int flag) {
        super(id, position, (short) 1, flag, uniqueid, (short) 0);
    }
    ...
    public int calStarForceStat(EquipStat stat) {
        int addStat = 0;
        Map<EchantEquipStat, Integer> echantStats;
        for (int i = 0; i < enhance; i++) {
            echantStats = EquipmentEnchant.getStarForceStats(i, getItemId(),
                    getWatk(0xFFFF - EquipStatsType.STAR_FORCE.getValue()),
                    getMatk(0xFFFF - EquipStatsType.STAR_FORCE.getValue()),
                    getWdef(0xFFFF - EquipStatsType.STAR_FORCE.getValue()), getReqLevel());
            switch (stat) {
                case WATK:
                    addStat += echantStats.getOrDefault(EchantEquipStat.WATK, 0);
                    break;
                case MATK:
                    addStat += echantStats.getOrDefault(EchantEquipStat.MATK, 0);
                    break;
                case STR:
                    addStat += echantStats.getOrDefault(EchantEquipStat.STR, 0);
                    break;
                case DEX:
                    addStat += echantStats.getOrDefault(EchantEquipStat.DEX, 0);
                    break;
                case INT:
                    addStat += echantStats.getOrDefault(EchantEquipStat.INT, 0);
                    break;
                case LUK:
                    addStat += echantStats.getOrDefault(EchantEquipStat.LUK, 0);
                    break;
                case WDEF:
                    addStat += echantStats.getOrDefault(EchantEquipStat.WDEF, 0);
                    break;
                case MDEF:
                    addStat += echantStats.getOrDefault(EchantEquipStat.MDEF, 0);
                    break;
                case MHP:
                    addStat += echantStats.getOrDefault(EchantEquipStat.MHP, 0);
                    break;
                case MMP:
                    addStat += echantStats.getOrDefault(EchantEquipStat.MMP, 0);
                    break;
                case ACC:
                    addStat += echantStats.getOrDefault(EchantEquipStat.ACC, 0);
                    break;
                case AVOID:
                    addStat += echantStats.getOrDefault(EchantEquipStat.AVOID, 0);
                    break;
                case JUMP:
                    addStat += echantStats.getOrDefault(EchantEquipStat.JUMP, 0);
                    break;
                case SPEED:
                    addStat += echantStats.getOrDefault(EchantEquipStat.SPEED, 0);
                    break;
            }
        }
        return addStat;
    ...
    public int getFlameStat(EquipStat stat) {
        return flameStats.getOrDefault(stat, 0);
    }

    private int calAddStat(int flag, EquipStat stat) {
        int value = 0;
        if (EquipStatsType.STAR_FORCE.check(flag)) {
            value += calStarForceStat(stat);
        }
        if (EquipStatsType.FLAME.check(flag)) {
            value += getFlameStat(stat);
        }
        return value;
    }

    public short getStr(int flag) {
        return (short) (str + calAddStat(flag, EquipStat.STR));
    }

    public short getDex(int flag) {
        return (short) (dex + calAddStat(flag, EquipStat.DEX));
    }

    public short getInt(int flag) {
        return (short) (_int + calAddStat(flag, EquipStat.INT));
    }

    public short getLuk(int flag) {
        return (short) (luk + calAddStat(flag, EquipStat.LUK));
    }

    public short getHp(int flag) {
        return (short) (hp + calAddStat(flag, EquipStat.MHP));
    }

    public short getMp(int flag) {
        return (short) (mp + calAddStat(flag, EquipStat.MMP));
    }

    public short getWatk(int flag) {
        return (short) (watk + calAddStat(flag, EquipStat.WATK));
    }

    public short getMatk(int flag) {
        return (short) (matk + calAddStat(flag, EquipStat.MATK));
    }

    public short getWdef(int flag) {
        return (short) (wdef + calAddStat(flag, EquipStat.WDEF));
    }

    public short getHands(int flag) {
        return (short) (hands + calAddStat(flag, EquipStat.HANDS));
    }

    public short getSpeed(int flag) {
        return (short) (speed + calAddStat(flag, EquipStat.SPEED));
    }

    public short getJump(int flag) {
        return (short) (jump + calAddStat(flag, EquipStat.JUMP));
    }
    ...
    public byte getBossDamage(int flag) {
        return (byte) (bossDamage + calAddStat(flag, EquipStat.BOSS_DAMAGE));
    }
    ...
    public byte getIgnorePDR(int flag) {
        return (byte) (ignorePDR + calAddStat(flag, EquipStat.IGNORE_PDR));
    }
    ...
    public byte getTotalDamage(int flag) {
        return (byte) (totalDamage + calAddStat(flag, EquipStat.TOTAL_DAMAGE));
    }
    ...
    public byte getAllStat(int flag) {
        return (byte) (allStat + calAddStat(flag, EquipStat.ALL_STAT));
    }
    ...
    public String getPotentialStr(int num, boolean isBonus) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        List<StructItemOption> potList = ii.getPotentialInfo(getPotential(num, isBonus));
        if (potList == null || potList.isEmpty()) {
            return "";
        }
        int lv = ii.getReqLevel(getItemId());
        StructItemOption pot = potList.get(Math.min(Math.max((lv / 10) - 1, 0), 20));
        String string = pot.string;
        List<String> pots = new LinkedList<>();
        Collections.addAll(pots, pot.getItemOption());
        String[] strs = string.split("#");
        if (strs.length - 1 > pots.size()) {
            return "";
        }
        string = "";
        for (int i = 0; i < strs.length; i++) {
            ...
            String ss = strs[i];
            for (String s : new LinkedList<>(pots)) {
                strs[i] = strs[i].replaceFirst(s, String.valueOf(pot.data.get(s)));
                if (!ss.equals(strs[i])) {
                    string += strs[i];
                    pots.remove(s);
                    break;
    ...
    public byte getFlame() {
        return flame;
    }

    public void setFlame(byte flame) {
        this.flame = flame;
    }
    ...
    public void resetPotential_Fuse(boolean half, int potentialState, boolean bonus) { // maker skill - equip first
        // receive
        // no legendary, 0.16% chance unique, 4% chance epic, else rare
        potentialState = -potentialState;
        if (Randomizer.isSuccess(4)) {
            potentialState -= Randomizer.isSuccess(4) ? 2 : 1;
        }
    ...
    public void resetPotential(int state, boolean fullLine, boolean bonus) {
        ...
        switch (state) {
            ...
            case 4:
                ...
                break;
            default:
                rank = Randomizer.isSuccess(4) ? (Randomizer.isSuccess(4) ? -19 : -18) : -17;
        }
        fullLine = getState(bonus) != 0 && getPotential(3, bonus) != 0 ? true : fullLine;
        setPotential(rank, 1, bonus);
        setPotential(Randomizer.isSuccess(10) || fullLine ? rank : 0, 2, bonus); // 1/10 chance of 3 line
        setPotential(0, 3, bonus); // just set it theoretically
        updateState(bonus);
    }
    ...
    public void renewPotential(int rate, int type, int toLock) {
        ...
        int rank = Randomizer.isSuccess(rate * miracleRate) ? 1 : 0;
        if (ItemConstants.方塊.CubeType.等級下降.check(type)) {
            if (rank == 0) {
                rank = Randomizer.isSuccess((rate + 20) * miracleRate) ? -1 : 0;
            }
        }
        ...
        if (getState(bonus) + rank < 17 || getState(bonus) + rank > (!ItemConstants.方塊.CubeType.傳說.check(type)
                ? !ItemConstants.方塊.CubeType.罕見.check(type) ? !ItemConstants.方塊.CubeType.稀有.check(type) ? 17 : 18 : 19
                : 20)) {
            rank = 0;
        }
        ...
        if (ItemConstants.方塊.CubeType.洗後無法交易.check(type)) {
            setFlag(getFlag() | ItemFlag.UNTRADABLE.getValue());
        }
    ...
    public static Equip calculateEquipStats(Equip eq) {
        ...
        if (eq.getBossDamage(0xFFFF) > 0) {
            eq.getStats().add(EquipStat.BOSS_DAMAGE);
        }
        if (eq.getIgnorePDR(0xFFFF) > 0) {
            eq.getStats().add(EquipStat.IGNORE_PDR);
        }
        if (eq.getTotalDamage(0xFFFF) > 0) {
            eq.getStats().add(EquipStat.TOTAL_DAMAGE);
        }
        if (eq.getAllStat(0xFFFF) > 0) {
            eq.getStats().add(EquipStat.ALL_STAT);
        }
        eq.getStats().add(EquipStat.KARMA_COUNT);
        if (eq.getFlame() > 0) {
            eq.getStats().add(EquipStat.FLAME);
        }
        if (eq.getStarForce() > 0) {
            eq.getStats().add(EquipStat.STAR_FORCE);
        }
    ...
    public Equip copyTo(Equip ret) {
        ...
        ret.stats.clear();
        ret.stats.addAll(stats);
        ret.flameStats.clear();
        ret.flameStats.putAll(flameStats);
        return ret;
    }

    public Item inheritance(Equip oldEquip) {
        if (ItemFlag.UNTRADABLE.check(oldEquip.getFlag())) {
            addFlag(ItemFlag.UNTRADABLE.getValue());
        }
        oldEquip.enhanctBuff = (short) ((enhanctBuff | oldEquip.enhanctBuff)
                - EquipStat.EnhanctBuff.EQUIP_MARK.getValue());
    ...
    public Item reset(Equip newEquip, boolean prefectReset) {
        ...
        if (prefectReset) {
            ...
            if (ItemFlag.UNTRADABLE.check(getFlag())) {
                newEquip.setFlag(getFlag() - ItemFlag.UNTRADABLE.getValue());
            }
            // 去除楓方塊狀態，否則回真後無法交易裝備又不能剪刀，需要穿一次裝才能剪刀(這個是台版BUG)
            if (ItemFlag.MAPLE_CUBE.check(getFlag())) {
                newEquip.setFlag(getFlag() - ItemFlag.MAPLE_CUBE.getValue());
            }
        } else {
