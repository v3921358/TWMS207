...
public class MaplePet implements Serializable {
    ...
    private String name;
    private int Fh = 0, stance = 0, uniqueid, petitemid, limitedLife = 0, skillid;
    private Point pos;
    ...
    public static final MaplePet loadFromDb(final int itemid, final int petid, final short inventorypos) {
        try {
            ...
            Connection con = ManagerDatabasePool.getConnection(); // Get a connection to the database
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM pets WHERE petid = ?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    ret.setLimitedLife(rs.getInt("limitedLife"));
    ...
    public final void saveToDb() {
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement(
                    "UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, limitedLife = ?, flags = ?, skillid = ?, excluded = ? WHERE petid = ?")) {
    ...
    public static final MaplePet createPet(final int itemid, final int uniqueid) {
        ...
        // 寵物技能修正為台版官服預設值
        return createPet(itemid, ii.getName(itemid), 1, 0, 100, uniqueid, ii.getLimitedLife(itemid),
                ii.getPetFlagInfo(itemid), 0);
    }

    public static final MaplePet createPet(int itemid, String name, int level, int closeness, int fullness,
            int uniqueid, int limitedLife, short flag, int skillId) {
        if (uniqueid <= -1) { // wah
            uniqueid = MapleInventoryIdentifier.getInstance();
        }
        flag = FeaturesConfig.PET_DEFAULT_FLAG >= 0 ? FeaturesConfig.PET_DEFAULT_FLAG : flag;
        try {
            Connection con = ManagerDatabasePool.getConnection();
            try (PreparedStatement pse = con.prepareStatement(
                    "INSERT INTO pets (petid, name, level, closeness, fullness, limitedLife, flags, skillid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public final int getLimitedLife() {
        return limitedLife;
    }

    public final void setLimitedLife(int ll) {
        this.limitedLife = ll;
        this.changed = true;
    }
    ...
    public static void clearPet() {
        try {
            ...
            PreparedStatement ps = con.prepareStatement(
                    "SELECT pets.petid, pets.name, inventoryitems.uniqueid FROM inventoryitems RIGHT JOIN pets ON pets.petid = inventoryitems.uniqueid where inventoryitems.uniqueid IS NULL");
            ...
            System.err.println("[MaplePet] 從數據庫中加載寵物訊息出錯" + se.getMessage());
        }
    }
}
