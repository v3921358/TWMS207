...
public class ItemConstants {
    ...
    public static class 傷害字型 {
        ...
        public static int getDamageSkinItemIDBySkinID(int skinID) {
            Map<Integer, Integer> skin = getDamageSkin();
            for (Map.Entry<Integer, Integer> s : skin.entrySet()) {
                if (s.getValue() == skinID) {
                    return s.getKey();
                }
            }
            return -1;
    ...
    public static class 方塊 {
        ...
        public static boolean canUseCube(Equip eq, int cubeId) {
            ...
            switch (cubeId) {
                case 2711007:// 10週年武器專用方塊
                    ...
                    if (set != null && set.itemIDs.contains(eq.getItemId()) && 類型.武器(eq.getItemId())) {
                        return true;
                    }
                    return false;
                case 5062100:// 楓葉奇幻方塊(罕見)
                    set = ii.getSetItem(33); // 緋紅楓葉組 [7週年]
                    if (set != null && set.itemIDs.contains(eq.getItemId()) && eq.getState(false) < 20) {
                        return true;
                    }
                    return false;
                case 5062102:// [6週年]奇幻方塊
                    int[] setArrays = {1462116, 1342039, 1402109, 1472139, 1332147, 1322105, 1442135, 1452128, 1312071,
                        1382123, 1492100, 1372099, 1432098, 1422072, 1302172, 1482101, 1412070};
                    List<Integer> list = new ArrayList<>();
                    for (int i : setArrays) {
                        list.add(i);
                    }
                    if (list.contains(eq.getItemId()) && 類型.武器(eq.getItemId())) {
                        return true;
                    }
                    return false;
                case 5062103:// 梦幻的神奇魔方
                    set = ii.getSetItem(100); // 十週年組 [8週年](實際是韓服的十週年,台服8週年)
                    StructSetItem set2 = ii.getSetItem(103); // 十週年高級組 [8週年](實際是韓服的十週年,台服8週年)
                    if (set != null && set2 != null && set.itemIDs.contains(eq.getItemId())
                            && set2.itemIDs.contains(eq.getItemId())) {
                        return true;
                    }
                    return false;
                case 2711000:// 可疑的方塊
        ...
        public enum CubeType {
            ...
            特殊(0x1), 稀有(0x2), 罕見(0x4), 傳說(0x8), 等級下降(0x10), 調整潛能條數(0x20), 洗後無法交易(0x40), 對等(0x80), 去掉無用潛能(0x100), 前兩條相同(
                    0x200), 附加潛能(0x400), 點商光環(0x800),;
        ...
        public static int getCubeType(int itemId) {
            int type = CubeType.特殊.getValue() | CubeType.稀有.getValue() | CubeType.罕見.getValue()
                    | CubeType.傳說.getValue();
            switch (itemId) {
        ...
        public static boolean potentialIDFits(final int potentialID, final int newstate, final int i) {
            ...
            switch (newstate) {
                ...
                case 19:
                    return (i == 1 || Randomizer.nextInt(20) == 0 ? potentialID >= 30000 && potentialID < 40000
                            : potentialID >= 20000 && potentialID < 30000);
                case 18:
                    return (i == 1 || Randomizer.nextInt(20) == 0 ? potentialID >= 20000 && potentialID < 30000
                            : potentialID >= 10000 && potentialID < 20000);
                case 17:
                    return (i == 1 || Randomizer.nextInt(20) == 0 ? potentialID >= 10000 && potentialID < 20000
                            : potentialID < 10000);
                default:
                    return false;
            }
        ...
        public static boolean isAllowedPotentialStat(Equip eqp, int opID, boolean bonus, boolean cash) { // For now
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            boolean superPot = FeaturesConfig.superiorPotential && ii.isSuperiorEquip(eqp.getItemId())
            ...
            int state = opID % 1000;
            return superPot && !bonus ? (state != 4 && state != 9 && state != 24 && (state < 13 || state > 18))
                    : opID < 60000;
        }
    ...
    public static class 類型 {
        ...
        public static boolean 重拳槍(final int itemid) {
        ...
        public static boolean 飾品(int itemid) {
            return 臉飾(itemid) || 眼飾(itemid) || 耳環(itemid) || 戒指(itemid) || 墜飾(itemid) || 腰帶(itemid) || 勳章(itemid)
                    || 肩飾(itemid) || 口袋道具(itemid) || 胸章(itemid) || 能源(itemid) || 圖騰(itemid);
        }
        ...
        public static boolean 武器(int itemid) {
            return 閃亮克魯(itemid) || 靈魂射手(itemid) || 魔劍(itemid) || 能量劍(itemid) || 幻獸棍棒(itemid) || ESP限制器(itemid)
                    || 單手劍(itemid) || 單手斧(itemid) || 單手棍(itemid) || 短劍(itemid) || 手杖(itemid) || 短杖(itemid) || 長杖(itemid)
                    || 雙手劍(itemid) || 雙手斧(itemid) || 雙手棍(itemid) || 槍(itemid) || 矛(itemid) || 弓(itemid) || 弩(itemid)
                    || 拳套(itemid) || 指虎(itemid) || 火槍(itemid) || 雙弩槍(itemid) || 加農炮(itemid) || 太刀(itemid) || 扇子(itemid)
                    || 璃(itemid);
        }
        ...
        public static boolean 雙手武器(final int itemid) {
            return 雙手劍(itemid) || 雙手斧(itemid) || 雙手棍(itemid) || 槍(itemid) || 矛(itemid) || 弓(itemid) || 弩(itemid)
                    || 拳套(itemid) || 指虎(itemid) || 火槍(itemid) || 雙弩槍(itemid) || 加農炮(itemid) || 太刀(itemid) || 扇子(itemid)
                    || 琉(itemid) || 璃(itemid);
        }
    ...
    public static boolean isOverPoweredEquip(final MapleClient c, final int itemId, short slot) {
        ...
        return source.getStr(0xFFFF) > 500 || source.getDex(0xFFFF) > 500 || source.getInt(0xFFFF) > 500
                || source.getLuk(0xFFFF) > 500 || source.getEnhance() > 25 || source.getHands(0xFFFF) > 100
                || source.getHp(0xFFFF) > 5000 || source.getMp(0xFFFF) > 5000 || source.getJump(0xFFFF) > 100
                || source.getSpeed(0xFFFF) > 100 || source.getMatk(0xFFFF) > 1200 || source.getUpgradeSlots() > 32
                || source.getViciousHammer() > 1 || source.getWatk(0xFFFF) > 1200
                || source.getWdef(0xFFFF) > 1700 /* || source.getLevel() > 32 */;
    }
    ...
    public static int getItemPrice(int itemId, int quantity) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        double price = Math.max(ii.getPrice(itemId), 0.0);
        if (類型.可充值道具(itemId)) {
            price += Math.ceil(quantity * ii.getUnitPrice(itemId));
        } else {
            price = Math.ceil(price * quantity);
        }
        return (int) Math.max(price, 1.0D);
    }

    public static int getItemGenderUnk(int itemId) {
        int gender = -1;
        switch (itemId) {
            case 2542001: // 精靈翼
            case 2542005: // 精靈翼
    ...
    public static int getFamiliarByItemID(int i) {
        return server.MapleItemInformationProvider.getInstance().getFamiliarID(i);
    }

    public static boolean isOldMonsterCard(int nItemId) {
