...
public class ShutdownServer implements ShutdownServerMBean {
    ...
    public void run() {
         ...
         * HiredMerchantSave.Execute(this); synchronized (this) { try { wait(); } catch
         * (InterruptedException ex) {
         * Logger.getLogger(ShutdownServer.class.getName()).log(Level.SEVERE, null, ex);
         * } } while (FinishedThreads.incrementAndGet() !=
         * HiredMerchantSave.NumSavingThreads) { synchronized (this) { try { wait(); }
