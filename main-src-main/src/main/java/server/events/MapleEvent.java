...
public abstract class MapleEvent {
    ...
    public void broadcast(int mapid, final byte[] packet) {
        getMap(mapid).broadcastMessage(packet);
    }
    ...
    public static String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(0,
                "Hello " + cserv.getServerName() + "的玩家們! 要參加 " + StringUtil.makeEnumHumanReadable(event.name())
                + " 活動的玩家請到頻道" + cserv.getChannel() + "！換到頻道" + cserv.getChannel() + " 後請使用 @event 指令！"));
        return "";
    }
