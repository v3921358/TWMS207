...
public class MapleMonsterInformationProvider {
    ...
    public void load() {
        ...
        try {
            ...
            while (rs.next()) {
                globaldrops.add(
                        new MonsterGlobalDropEntry(rs.getInt("itemid"), rs.getInt("chance"), rs.getInt("continent"),
                                rs.getByte("dropType"), rs.getInt("minimum_quantity"), rs.getInt("maximum_quantity"),
                                rs.getInt("questid"), rs.getInt("elite"), rs.getInt("channelType"), "數據庫"));
            }
            rs.close();
            ps.close();
    ...
    public void loadCustom() {// 讀取自定義全域掉寶
        // globaldrops.add(new MonsterGlobalDropEntry(4001126, 5 * 10000, -1, (byte) 0,
        // 1, 3, 0, "源碼自定義")); //楓葉
        // globaldrops.add(new MonsterGlobalDropEntry(4310050, (int) (1 * 10000), -1,
        // (byte) 0, 1, 1, 0, "源碼自定義")); //Old Maple Coin
        // globaldrops.add(new MonsterGlobalDropEntry(2290285, (int) (0.5 * 10000), -1,
        // (byte) 0, 1, 1, 0, "源碼自定義")); //[百科書]神秘百科書
        // globaldrops.add(new MonsterGlobalDropEntry(4000524, 5 * 10000, -1, (byte) 0,
        // 1, 1, 0, "源碼自定義"));
    }
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM drop_data WHERE dropperid = ?")) {
                ...
                while (rs.next()) {
                    ...
                    ret.add(new MonsterDropEntry(itemid, chance, rs.getInt("minimum_quantity"),
                            rs.getInt("maximum_quantity"), rs.getInt("questid"), rs.getInt("channelType"), "數據庫"));
                    if (itemid == 0) {
                        doneMesos = true;
                    }
                }
    ...
    public void addExtra() {
        ...
        for (Entry<Integer, ArrayList<MonsterDropEntry>> e : drops.entrySet()) {
            ...
            if (item != null && item > 0) {
                if (item / 10000 == 238 || mons == null) {// 過濾怪物卡掉寶數據
                    continue;
                }
                e.getValue().add(new MonsterDropEntry(item, mons.isBoss() ? 1000000 : 10000, 1, 1, 0, 0, "道具數據庫"));
            }
            StructFamiliar f = ii.getFamiliarByMob(e.getKey());
            if (f != null) {
                if (f.getMonsterCardId() / 10000 == 238 || mons == null) {// 過濾怪物卡掉寶數據
                    continue;
                }
                e.getValue().add(new MonsterDropEntry(f.getMonsterCardId(), mons.isBoss() ? 10000 : 100, 1, 1, 0, 0,
                        "FamiliarInfo"));
            }
        }
        for (Entry<Integer, Integer> i : ii.getMonsterBook().entrySet()) {
            if (!drops.containsKey(i.getKey())) {
                ...
                if ((i.getValue()) / 10000 == 238 || mons == null) {// 過濾怪物卡掉寶數據
                    continue;
                }
                e.add(new MonsterDropEntry(i.getValue(), mons.isBoss() ? 1000000 : 10000, 1, 1, 0, 0, "道具數據庫2"));
                StructFamiliar f = ii.getFamiliarByMob(i.getKey());
                if (f != null) {
                    if (f.getMonsterCardId() / 10000 == 238) {// 過濾怪物卡掉寶數據
                        continue;
                    }
                    e.add(new MonsterDropEntry(f.getMonsterCardId(), mons.isBoss() ? 10000 : 100, 1, 1, 0, 0,
                            "FamiliarInfo2"));
                }
                addMeso(mons, e);
        ...
        for (StructFamiliar f : ii.getFamiliars().values()) {
            if (!drops.containsKey(f.getMonsterCardId())) {
                MapleMonsterStats mons = MapleLifeFactory.getMonsterStats(f.getMobId());
                ArrayList<MonsterDropEntry> e = new ArrayList<>();
                if (ItemConstants.isOldMonsterCard(f.getMonsterCardId()) || mons == null) {// 過濾怪物卡掉寶數據
                    continue;
                }
                e.add(new MonsterDropEntry(f.getMonsterCardId(), mons.isBoss() ? 10000 : 100, 1, 1, 0, 0,
                        "FamiliarInfo3"));
                addMeso(mons, e);
                drops.put(f.getMobId(), e);
            }
        }
        for (Entry<Integer, ArrayList<MonsterDropEntry>> e : drops.entrySet()) {// 怪物書掉寶數據
            if (mobStringData.getChildByPath(String.valueOf(e.getKey())) != null) {
                for (MapleData d : mobStringData.getChildByPath(e.getKey() + "/reward").getChildren()) {
                    final int toAdd = MapleDataTool.getInt(d, 0);
                    if (toAdd > 0 && !contains(e.getValue(), toAdd) && ii.itemExists(toAdd)) {
                        if (toAdd / 10000 != 238 && toAdd / 10000 != 243 && toAdd / 10000 != 399 && toAdd != 4001126
                                && toAdd != 4001128 && toAdd != 4001246 && toAdd != 4001473 && toAdd != 2022450
                                && toAdd != 2022451 && toAdd != 2022452 && toAdd != 4032302 && toAdd != 4032303
                                && toAdd != 4032304) {
                            e.getValue().add(new MonsterDropEntry(toAdd, chanceLogic(toAdd), 1, 1, 0, 0, "怪物書"));
                        }
                    }
                }
            }
    ...
    public void addMeso(MapleMonsterStats mons, ArrayList<MonsterDropEntry> ret) {
        ...
        for (int i = 0; i < mons.dropsMeso(); i++) {
            ret.add(new MonsterDropEntry(0,
                    ...
                    (int) Math.floor(0.66 * max), max, 0, 0, "楓幣添加"));
        }
    }
    ...
    public Map<Integer, String> getAllMonsters() {
        if (mobCache.isEmpty()) {
            final MapleDataProvider stringData = MapleDataProviderFactory.getDataProvider("String");
            MapleData mobsData = stringData.getData("Mob.img");
            for (MapleData itemFolder : mobsData.getChildren()) {
                mobCache.put(Integer.parseInt(itemFolder.getName()),
    ...
    public int chanceLogic(int itemId) {
        if (null == GameConstants.getInventoryType(itemId)) {
            switch (itemId / 10000) {
                case 204:
                case 207:
                case 233:
