...
public class MapleLifeFactory {
    ...
    private static final MapleDataProvider[] data = new MapleDataProvider[]{
        MapleDataProviderFactory.getDataProvider("Mob"), MapleDataProviderFactory.getDataProvider("Mob2"),};
    private static final MapleDataProvider npcData = MapleDataProviderFactory.getDataProvider("Npc");
    private static final MapleDataProvider stringDataWZ = MapleDataProviderFactory.getDataProvider("String");
    private static final MapleDataProvider etcDataWZ = MapleDataProviderFactory.getDataProvider("Etc");
    ...
    public static void loadQuestCounts(boolean reload) {
        ...
        for (MapleDataProvider mdp : data) {
            src = mdp;
            if (src == null) {
                continue;
            }
            for (MapleDataDirectoryEntry mapz : src.getRoot().getSubdirectories()) {
                if (mapz.getName().equals("QuestCountGroup")) {
                    for (MapleDataFileEntry entry : mapz.getFiles()) {
                        ...
                        MapleData dat = src.getData("QuestCountGroup/" + entry.getName());
                        if (dat != null && dat.getChildByPath("info") != null) {
                            List<Integer> z = new ArrayList<>();
                            for (MapleData da : dat.getChildByPath("info").getChildren()) {
        ...
        /*
         * for (MapleData c : npcStringData) { int nid = Integer.parseInt(c.getName());
         * String n = StringUtil.getLeftPaddedStr(nid + ".img", '0', 11); try { if
         * (npcData.getData(n) != null) {//only thing we really have to do is check if
         * it exists. if we wanted to, we could get the script as well :3 String name =
         * MapleDataTool.getString("name", c, "MISSINGNO"); if
         * (name.contains("Maple TV") || name.contains("Baby Moon Bunny")) { continue; }
         * npcNames.put(nid, name); } } catch (NullPointerException e) { } catch
         * (RuntimeException e) { //swallow, don't add if } }
         */
        try {
    ...
    public static MapleMonsterStats getMonsterStats(int mid) {
        ...
        if (stats == null) {
            ...
            MapleDataProvider src = null;
            for (MapleDataProvider dat : data) {
                try {
                    src = dat;
                    if (src == null) {
                        break;
                    }
                    monsterData = src.getData(StringUtil.getLeftPaddedStr(Integer.toString(mid) + ".img", '0', 11));
            ...
            if (monsterData == null || src == null) {
                return null;
            }
            MapleData monsterInfoData = monsterData.getChildByPath("info");
            stats = new MapleMonsterStats(mid);

            stats.setHp((int) (GameConstants.getPartyPlayHP(mid) > 0 ? GameConstants.getPartyPlayHP(mid)
                    : MapleDataTool.getLongConvert("maxHP", monsterInfoData, 50000000)));
            stats.setFinalMaxHP(MapleDataTool.getLongConvert("finalmaxHP", monsterInfoData, 0));
            stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 50000000));
            ...
            stats.setSkeleton(MapleDataTool.getIntConvert("skeleton", monsterInfoData, 0));
            ...
            if (reviveInfo != null) {
                ...
                for (MapleData bdata : reviveInfo.getChildren()) {
            ...
            if (link != 0) { // Store another copy, for faster processing.
                monsterData = src.getData(StringUtil.getLeftPaddedStr(link + ".img", '0', 11));
                if (monsterData == null) {
                    return null;
                }
            }
            OUTER:
            for (MapleData idata : monsterData.getChildren()) {
            ...
            for (int i = 0; true; i++) {
                ...
                ret.magic = magic;
                ret.isElement = monsterAtt.getChildByPath("elemAttr") != null; // we handle it like this, i don't know
                // what it does

                if (attackData.getChildByPath("range") != null) { // Definitely in attackData
                    ret.range = MapleDataTool.getInt("range/r", attackData, 0);
                    if (attackData.getChildByPath("range/lt") != null
                            && attackData.getChildByPath("range/rb") != null) {
