...
public class CharacterCardFactory {
    ...
    public void initialize(boolean reload) {
        ...
        // System.out.println("Loaded " + (cardEffects.size() + uniqueEffects.size()) +
        // " card effects");
    }

    public final Triple<Integer, Integer, Integer> getCardSkill(final int job, final int level) { // cardid, skillid,
        // skilllevel
            ...
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM `character_cards` WHERE `accid` = ?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    while (rs.next()) {
                        ...
                        if (x == null || !canHaveCard(x.getLeft(), x.getRight())) { // we don't need to delete them as
                            // it'll be there till the user
                            // reupdate the char cards
                            continue;
                        }
