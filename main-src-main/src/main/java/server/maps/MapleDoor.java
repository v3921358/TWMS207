...
public class MapleDoor extends MapleMapObject {
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        if (target.getId() == client.getPlayer().getMapId()) {
            client.getSession()
                    .writeAndFlush(CField.spawnDoor(getOwnerId(), getSkill(),
                            target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition(),
                            target.getId() == client.getPlayer().getMapId() ? first : false)); // spawnDoor always has
            // same position.
        } else if (town.getId() == client.getPlayer().getMapId()) {
            if (getOwnerId() == client.getPlayer().getId() && client.getPlayer().getParty() == null) {
                client.getSession().writeAndFlush(CWvsContext.spawnPortal(town.getId(), target.getId(), skillId,
                        target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition()));
            } else {
                client.getSession().writeAndFlush(PartyPacket.partyPortal(town.getId(), target.getId(), skillId,
                        target.getId() == client.getPlayer().getMapId() ? targetPosition : townPortal.getPosition(),
                        first));
            }
        }
    ...
    public final void sendDestroyData(final MapleClient client) {
        ...
        if (target.getId() == client.getPlayer().getMapId() || getOwnerId() == client.getPlayer().getId()
            ...
            if (getOwner() != null && getOwner().getParty() != null && client.getPlayer().getParty() != null
                    ...
                    || getOwner().getParty().getId() == client.getPlayer().getParty().getId())) {
                client.getSession()
                        .writeAndFlush(PartyPacket.partyPortal(999999999, 999999999, 0, new Point(-1, -1), false));
            }
