...
public class Event_PyramidSubway {
            ...
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    ...
                    chr.getClient().getSession()
                            .writeAndFlush(CField.showScreenAutoLetterBox("killing/first/number/" + stage));
                    chr.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/first/stage"));
                    chr.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().getSession().writeAndFlush(CField.getClock(time));
            c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/first/number/" + stage));
            c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/first/stage"));
            c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/first/start"));
            fullUpdate(c, stage);
        }
    ...
    public final void onKill(final MapleCharacter c) {
        kill++;
        bar++;
        if (Randomizer.isSuccess(5)) { // monster properties coolDamage and coolDamageProb determine this, will code
            // later
            cool++;
            bar++;
            broadcastEnergy(c, "massacre_cool", cool);
        ...
        if (type != -1) {
            for (int i = 5; i >= 1; i--) {
                if ((kill + cool) % (i * 100) == 0 && Randomizer.isSuccess(20)) {
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/clear"));
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().getSession().writeAndFlush(CField.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox(effect));
    }
    ...
    private static void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel,
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel
                    ...
                    if (clear == 1) {
                        chr.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().getSession().writeAndFlush(CField.showScreenAutoLetterBox("killing/fail"));
        }
