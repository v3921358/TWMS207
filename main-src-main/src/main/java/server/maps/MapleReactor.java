        ...
        if (stats.getType(state) < 999 && stats.getType(state) != -1) {
            ...
            if (!(stats.getType(state) == 2 && (charPos == 0 || charPos == 2))) { // next state
                ...
                if (stats.getNextState(state) == -1 || stats.getType(state) == 999) { // end of reactor
                    ...
                    boolean done = false;
                    map.broadcastMessage(CField.triggerReactor(c.getPlayer(), this, stance)); // magatia is weird cause
                    // full beaker can be
                    // activated by gm hat o.o
                    if (state == stats.getNextState(state) || rid == 2618000 || rid == 2309000) { // current state =
                        // next state, looping
                        // reactor
                        if (rid > 200011) {
                            ReactorScriptManager.getInstance().act(c, this);
