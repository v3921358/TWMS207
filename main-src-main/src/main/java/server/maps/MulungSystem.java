...
public class MulungSystem {

    private final static int BASE_MAP_ID = 970030000; // 9500337 = mano
    private final static List<Point> SPAWN_POINTS = Arrays.asList(new Point(140, 0), new Point(-193, 0),
            new Point(355, 0));

    public boolean isInMulungMap(MapleCharacter player) {
        return player.getMapId() / 10000 != 92502;
    }

    public static boolean warpStartAgent(final MapleCharacter c, final boolean party) {
        final int stage = 1;
        final int mapid = BASE_MAP_ID + (stage * 100);
    ...
    public static boolean warpNextMap_Agent(final MapleCharacter c, final boolean fromResting) {
        ...
        final int thisStage = (currentmap - BASE_MAP_ID) / 100;
        ...
        if (currentmap >= 970032700 && currentmap <= 970032800) {
            map = ch.getMapFactory().getMap(BASE_MAP_ID);
            c.changeMap(map, map.getPortal(0));
            return true;
        }
        final int nextmapid = BASE_MAP_ID + ((thisStage + 1) * 100);
    ...
    public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting, final MapleMap currentmap) {
        try {
            ...
            switch (currentmap.getId()) {
                ...
                case 925021100: { // End Normal
                    final MapleMap mappzz = ch.getMapFactory().getMap(925020003);// rooftop
                    c.changeMap(mappzz, mappzz.getPortal(1));
                    break;
                }
                case 925032300: { // End Hard
                    final MapleMap mappzz = ch.getMapFactory().getMap(925020003);// rooftop
                    c.changeMap(mappzz, mappzz.getPortal(1));
                    c.getClient().getSession().writeAndFlush(CField.UIPacket.openUI(72));// Dojo ui
                    break;
                }
                default: {
    ...
    private static void spawnMonster(final MapleMap map, final int stage) {
        ...
        if (mobid != 0) {
            MapTimer.getInstance().schedule(() -> {
                final Point spawnPoint = SPAWN_POINTS.get(Randomizer.nextInt(SPAWN_POINTS.size()));
                map.spawnMonsterWithEffect(MapleLifeFactory.getMonster(mobid), 15, spawnPoint);
            }, 3000);
        }
    }
}
