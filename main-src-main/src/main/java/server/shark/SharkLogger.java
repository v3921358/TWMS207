...
public class SharkLogger {
    ...
    public void dump() {
        ...
        if (MAPLE_SHARK_VERSION == 0x2012) {
            ...
            mplew.writeShort(Integer.parseInt(localend.split(":")[1])); // mLocalPort
            write7BitInt((ServerConstants.getHostAddress() + ":" + mRemotePort).length(), mplew);
            mplew.writeAsciiString(ServerConstants.getHostAddress() + ":" + mRemotePort); // mRemoteEndpoint
        ...
        File toWrite = new File(
                FileoutputUtil.Shark_Dir + (client.getPlayer() != null ? client.getPlayer().getName() + "_" : "")
                + client.getSessionId() + ".msb");
