...
public class MapleQuestRequirement implements Serializable {
    ...
    public boolean check(MapleCharacter c, Integer npcid) {
        switch (type) {
            ...
            case fieldEnter:
                String[] fields = stringStore.split(",");
                int store;
                for (String field : fields) {
                    store = Integer.parseInt(field);
                    if (store == 0 || store == c.getMapId()) {
                        return true;
                    }
                }
                return false;
            ...
            case subJobFlags: // 1 for non-DB, 2 for DB...
                return c.getSubcategory() == (intStore / 2)
                        || String.valueOf(intStore).contains(String.valueOf(c.getSubcategory())); // 應該是這樣修
            // return c.getSubcategory() == (intStore / 2)
            // || (quest.getId() == 32240 && c.getJob() / 100 > 0 && c.getJob() / 100 < 6)
            // || (quest.getId() == 2681);//判斷有點問題...
            case craftMin:
            case willMin:
            case charismaMin:
            case insightMin:
