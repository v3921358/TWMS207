    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
                ...
                for (QuestItem item : this.items) {
                    if (canGetItem(item, c)) {
                        ...
                        if ((item.prop == -2) || (item.prop == -1 ? (extSelection == null) && (extSelection == extNum++)
                            ...
                            if (count < 0) {
                                try {
                                    MapleInventoryManipulator.removeById(c.getClient(), GameConstants.getInventoryType(id),
                                            id, count * -1, true, false);
                                } catch (InventoryException ie) {
                                    System.err.println("[h4x] Completing a quest without meeting the requirements" + ie);
                                }
                                c.getClient().getSession()
                                        .writeAndFlush(CWvsContext.InfoPacket.getShowItemGain(id, count, true));
                            } else {
            ...
            case nextQuest:
                ...
                if (status.getForfeited() <= 0) {
                    c.getClient().getSession()
                            .writeAndFlush(CField.updateQuestFinish(this.quest.getId(), status.getNpc()));
                }
                break;
            case money:
                status = c.getQuest(this.quest);
        ...
        switch (type) {
            ...
            case nextQuest:
                c.getClient().getSession().writeAndFlush(
                        CField.updateQuestFinish(this.quest.getId(), c.getQuest(this.quest).getNpc()));
                break;
            case money:
                c.gainMeso(this.intStore, true, true);
                break;
