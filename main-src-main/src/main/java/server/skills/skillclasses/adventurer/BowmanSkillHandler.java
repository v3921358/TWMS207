 ...
 * @author Weber
 */
public class BowmanSkillHandler extends AbstractSkillHandler {
    ...
    public void onAttack(MapleCharacter player, AttackInfo attackInfo, List<AttackPair> attackPairs, Skill skill) {
        attackPairs.forEach(attackPair -> {
            ...
            if (player.getBuffedValue(MapleBuffStat.QuiverCatridge) != null) {
                player.handleQuiverKartrige(monster.getObjectId());
            }
        });
    }
    ...
    public boolean checkJob(MapleCharacter player) {
        return MapleJob.is弓箭手(player.getJob());
    }
