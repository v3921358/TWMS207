...
public class SkillPacket {
    ...
    public static byte[] sendEngagementRequest(String name, int chrId) {
        if (ServerConfig.LOG_PACKETS) {
            System.out.println(new StringBuilder().append("調用位置: ").append(new java.lang.Throwable().getStackTrace()[0])
                    .toString());
        }
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.LP_MarriageRequest.getValue());
        mplew.write(0);
    ...
    public static byte[] sendEngagement(byte msg, int item, MapleCharacter male, MapleCharacter female) {
        ...
        mplew.writeShort(SendPacketOpcode.LP_MarriageResult.getValue());
        mplew.write(msg);
    ...
    public static byte[] spawnOpenGate(MapleOpenGate md, boolean animated) {
        if (ServerConfig.LOG_PACKETS) {
            System.out.println(new StringBuilder().append("調用位置: ").append(new java.lang.Throwable().getStackTrace()[0])
                    .toString());
        }
    ...
    public static byte[] removeOpenGate(MapleOpenGate md, boolean animated) {
        if (ServerConfig.LOG_PACKETS) {
            System.out.println(new StringBuilder().append("調用位置: ").append(new java.lang.Throwable().getStackTrace()[0])
                    .toString());
        }
        ...
        mplew.writeShort(SendPacketOpcode.LP_OpenGateClose.getValue());
        mplew.write(animated ? 0 : 1);
        mplew.writeInt(md.getOwnerId());
        mplew.write(md.getId());
    ...
    public static byte[] getMesosBomb(MapleCharacter chr, int skillId, List<MapleMapObject> mesos,
            List<MapleMapObject> mons, int delay) { // 楓幣炸彈封包
        if (ServerConfig.LOG_PACKETS) {
    ...
    public static byte[] gainAssassinStack(int chrId, int oid, int forceCount, boolean isAssassin,
            List<Integer> moboids, int visProjectile, Point posFrom) {
        if (ServerConfig.LOG_PACKETS) {
            System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
        }
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] showQuiverKartrigeEffect(int chrid, int mode, int totle, boolean other) {
        ...
        mplew.write(UserEffectOpcode.UserEffect_PetBuff.getValue());
    ...
    public static byte[] showArmorPenetrat() { // 壓制 發送的效果包
        ...
        mplew.writeShort(SendPacketOpcode.LP_UserEffectLocal.getValue());
        mplew.write(UserEffectOpcode.UserEffect_SkillUse.getValue());
        mplew.writeInt(33120018);
        mplew.write(0xA0);
        mplew.write(1);
        mplew.write(1);
        return mplew.getPacket();
    ...
    public static byte[] AccurateRocket(MapleCharacter chr, int skillId, List<Integer> mons, int monsid,
            int BulletCount, int times, boolean isAegis) { // 傑諾追縱火箭 和 神盾系統
        if (ServerConfig.LOG_PACKETS) {
            System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
        }
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    // TODO 全息力场
    public static byte[] ForceFieldAttack(int id, int mapid, int level, Point cpoint, int side, Point spoint,
            boolean sp) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        /*
         * mplew.writeShort(SendPacketOpcode.ARROWS_TURRET_ATTACK.getValue());
         * mplew.writeShort(0); mplew.writeInt(id);//Object_id 第几个
         * mplew.writeInt(mapid); mplew.write(1); mplew.writeInt(1);
         * mplew.writeShort(5); mplew.writeInt(cpoint.x); mplew.writeInt(cpoint.y);
         * mplew.writeShort(14); mplew.writeInt(sp ? 0 : 655370); mplew.writeShort(14);
         * mplew.writeInt(skillid); mplew.write(side);//方向 mplew.writeShort(0);
         */
        return mplew.getPacket();
    }
    ...
    public static byte[] cancel标飞标记(int monOID, MapleCharacter chr) { // 标飞标记
        ...
        PacketHelper.writeSingleMobMask(mplew, MonsterStatus.M_Burned);
        mplew.writeInt(1);
        mplew.writeInt(1);
        mplew.writeInt(chr.getId());
    ...
    public static byte[] canLuckyMoney(boolean x) { // 翻轉硬幣是否可以使用
        ...
        mplew.writeShort(SendPacketOpcode.LP_DetonateBomb.getValue());
        mplew.write(x);
        return mplew.getPacket();
    }

    // public static byte[] getAdvancedAttack(int srcskill, int paskill, int
    // WeaponType, int value, int monoid) { //被动触发进阶攻击或被动攻击
    // MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    //
    ...
    // }
    //
    // public static byte[] AngelReborn(int srcskill, boolean done) {
    // //被动触发进阶攻击或被动攻击
    // MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    // if (done) {
    ...
    public static byte[] DrainSoul(MapleCharacter chr, int sourceOid, List<Integer> mon, int forceCount, int count,
            int skillid, int dell, boolean frommon, int dwTime) {// 灵魂吸取
        if (ServerConfig.LOG_PACKETS) {
            System.out.println("調用位置: " + new java.lang.Throwable().getStackTrace()[0]);
        }
        List<ForceAtom> forceInfo = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            forceInfo.add(chr.getNewAtom(skillid, frommon));
        }
        return CField.OnCreateForceAtom(frommon, chr, mon, 3, skillid, forceInfo, null, null, 0, 0, 0);
        // mplew.writeShort(SendPacketOpcode.LP_ForceAtomCreate.getValue());
        // mplew.write(frommon);
        // if (frommon) {
        // mplew.writeInt(chr.getId());
        // }
        // mplew.writeInt(frommon ? sourceOid : chr.getId());
        // mplew.writeInt(frommon ? 4 : 3);//type
        //
        // mplew.write(1);
        // if (frommon) {
        // mplew.writeInt(sourceOid);
        // } else {
        ...
        // mplew.writeInt(frommon ? Randomizer.rand(0x20, 0x30) : Randomizer.rand(0x0F,
        // 0x20));
        // mplew.writeInt(frommon ? Randomizer.rand(3, 4) : Randomizer.rand(0x15,
        // 0x30));
        // mplew.writeInt(frommon ? Randomizer.rand(0x0, 0xFF) : Randomizer.rand(0x30,
        // 0x50));
        // mplew.writeInt(dell);
        // mplew.writeInt(0);
        // mplew.writeInt(0);
        // mplew.writeInt(DateUtil.getTime(System.currentTimeMillis()));
        // mplew.writeInt(1);
        // mplew.writeInt(frommon ? 0 : (i == count - 1 ? 0 : 1));
        // mplew.writeInt(0);
        // }
        // mplew.write(0);
        // return mplew.getPacket();
    ...
    public static byte[] ConveyTo() {
        ...
        mplew.writeShort(SendPacketOpcode.LP_ExclRequest.getValue());
        mplew.write(1);
        return mplew.getPacket();
    }
}
