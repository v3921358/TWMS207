...
public class FarmPacket {
    ...
    public static byte[] enterFarm(MapleClient c) {
        ...
        mplew.writeShort(SendPacketOpcode.LP_SetFarmField.getValue());
        PacketHelper.CharacterData_Decode(mplew, c.getPlayer());
        MapleFarm f = c.getFarm();
        long time = System.currentTimeMillis();
        /*
         * Farm House positions: 000 001 002 003 004 025 026 027 028 029 050 051 052 053
         * 054 075 076 077 078 079 100 101 102 103 104 //104 is base position
         */
        ...
        mplew.writeInt(14);
        mplew.writeInt(0);
        mplew.writeLong(DateUtil.getFileTimestamp(time + 180000));
    ...
    public static byte[] spawnFarmMonster(MapleClient c, int id) {
        ...
        mplew.writeLong(DateUtil.getFileTimestamp(time));
        mplew.writeLong(DateUtil.getFileTimestamp(time + 25920000000000L));
        mplew.writeLong(DateUtil.getFileTimestamp(time + 25920000000000L));
        for (int i = 0; i < 4; i++) {
            mplew.writeLong(DateUtil.getFileTimestamp(time));
        }
    ...
    public static byte[] updateMonster(List<Pair<Integer, Long>> monsters) {
        ...
        for (Pair<Integer, Long> monster : monsters) {
            ...
            mplew.writeLong(DateUtil.getFileTimestamp(monster.getRight())); // expire
        }
    ...
    public static byte[] updateAvatar(Pair<WorldConstants.Option, MapleCharacter> from,
            Pair<WorldConstants.Option, MapleCharacter> to, boolean change) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
