...
public class MapleParty implements Serializable {
    ...
    private final ReentrantReadWriteLock visiblePartBuffsLock = new ReentrantReadWriteLock();
    ...
    public void givePartyBuff(int buffId, int applyfrom, int applyto) {
        visiblePartBuffsLock.writeLock().lock();
        try {
            if (partyBuffs.containsKey(buffId)) {
                if (partyBuffs.get(buffId).containsKey(applyfrom)) {
                    if (!partyBuffs.get(buffId).keySet().isEmpty()) {
                        for (Integer from : partyBuffs.get(buffId).keySet()) {
                            if (partyBuffs.get(buffId).get(from).contains(applyto)) {
                                partyBuffs.get(buffId).get(from)
                                        .remove(partyBuffs.get(buffId).get(from).indexOf(applyto));
                            }
                    ...
                    if (partyBuffs != null && !partyBuffs.get(buffId).isEmpty()
                            && !partyBuffs.get(buffId).get(applyfrom).isEmpty()
                            && !partyBuffs.get(buffId).get(applyfrom).contains(applyto)) {
                        partyBuffs.get(buffId).get(applyfrom).add(applyto);
                    }
                } else {
                    List<Integer> applytos = new ArrayList<>();
            ...
            visiblePartBuffsLock.writeLock().unlock();
        }
    }
                ...
                if (partyBuffs.get(buffId).isEmpty()) {
                    ...
                    for (Integer applyfrom : partyBuffs.get(buffId).keySet()) {
                        if (partyBuffs.get(buffId).get(applyfrom).isEmpty()) {
                            ...
                            partyBuffs.get(buffId).get(applyfrom)
                                    .remove(partyBuffs.get(buffId).get(applyfrom).indexOf(cancelby));
                            return applyfrom;
                        }
                    }
                }
