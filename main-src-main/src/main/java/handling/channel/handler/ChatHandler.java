    ...
    public static void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                // Note: This patch is needed to prevent chat packet from being broadcast to
                // people who might be packet sniffing.
                ...
                if (chr.isHidden()) {
                        ...
                        if (unk == 0) {
                            if (chr.isHiddenChatCanSee()) {
                                ...
                                chr.getMap().broadcastMessage(chr,
                                        chr.isIntern() ? colorChatPacket : colorCurseChatPacket, false);
                            } else {
                    ...
                    if (colorChatPacket == null) {
                        c.getSession().writeAndFlush(CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), unk));
                        chr.getMap().broadcastMessage(chr, CField.getChatText(chr.getId(), chr.isIntern() ? text : curseText, c.getPlayer().isSuperGM(), unk), false);
                    } else {
                        if (unk == 0) {
                            c.getSession().writeAndFlush(colorChatPacket);
                            chr.getMap().broadcastMessage(chr, chr.isIntern() ? colorChatPacket : colorCurseChatPacket,
                                    false);
                        }
                        c.getSession().writeAndFlush(CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), 1));
                        chr.getMap().broadcastMessage(chr, CField.getChatText(chr.getId(), chr.isIntern() ? text : curseText, c.getPlayer().isSuperGM(), 1), false);
                    }
                }
            } else {
                c.getSession().writeAndFlush(CWvsContext.broadcastMsg(6, "已被禁言, 無法說話"));
            }
        }
    }

    public static void AdminChat(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        if (!c.getPlayer().isIntern()) {// if ( (signed int)CWvsContext::GetAdminLevel((void *)v294) > 2 )
            return;
        }
        byte mode = slea.readByte();
        // not saving slides...
        byte[] packet = CWvsContext.broadcastMsg(slea.readByte(), slea.readMapleAsciiString());// maybe I should make a
        // check for the
        // slea.readByte()... but
        // I just hope gm's don't
        // fuck things up :)
        switch (mode) {
    ...
    public static void Others(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr == null || !chr.getCanTalk()) {
            c.getSession().writeAndFlush(
                    CWvsContext.broadcastMsg(6, "You have been muted and are therefore unable to talk."));
            return;
        }

        System.out.println(
                new StringBuilder().append("[聊天訊息] ").append(chr.getName()).append(" : ").append(chattext).toString());
        if (c.isMonitored()) {
            String chattype = "未知";
            switch (type) {
                case 0:
            ...
            World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, "[GM 訊息] "
                    + MapleCharacterUtil.makeMapleReadable(chr.getName()) + " 在 (" + chattype + ") 中說: " + chattext));
        ...
        switch (type) {
            ...
            default:
                System.err.println(
                        "未知操作類型: ( 0x" + StringUtil.getLeftPaddedStr(Integer.toHexString(type).toUpperCase(), '0', 2) + " )"
                        + slea.toString());
        }
    }
        ...
        switch (slea.readByte()) {
            ...
            case 0x06: // message
                if (messenger != null) {
                    ...
                    if (messenger.isMonitored() && chattext.length() > c.getPlayer().getName().length() + 3) { // name : NOT
                        // name0 or
                        // name1
                        World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6,
                                ...
                                + "(Messenger: " + messenger.getMemberNamesDEBUG() + ") said: " + chattext));
                    }
                }
                break;
            case 0x09: // like
        ...
        switch (mode) {
            ...
            case 5: { // 搜尋Find
                ...
                if (player != null) {
                    ...
                    } else if (ch == MapleServerHandler.CASH_SHOP_SERVER) {
            ...
            case 6: { // 悄悄話Whisper
                ...
                if (ch > 0) {
                    ...
                    player.getClient().getSession()
                            .writeAndFlush(CField.getWhisper(c.getPlayer().getName(), c.getChannel(), text));
            ...
            default: {
                System.err.println(
                        "未知操作類型: ( 0x" + StringUtil.getLeftPaddedStr(Integer.toHexString(mode).toUpperCase(), '0', 2) + " )"
                        + slea.toString());
            }
        }
    }
