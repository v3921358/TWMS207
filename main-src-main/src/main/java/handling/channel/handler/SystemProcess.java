...
public class SystemProcess {
        ...
        try {
            for (int i = 0; i < num; i++) {
                ...
                if (SystemProcess.checkWGProcess(process) != null
                        || SystemProcess.checkWGProcess(chr.getProcess()) != null) {
                    ...
                    c.getSession().writeAndFlush(CScriptMan.getNPCTalk(9010000, ScriptMessageType.SM_SAY,
                            "系統檢測到你使用非法程序，你將被永久封號。", "00 00", (byte) 0, 9010000));
                    Timer.CheatTimer.getInstance().schedule(new Runnable() {
                        @Override
                        public void run() {
                            chr.ban("被系統封號：使用外掛 " + WG.getName(), null, MapleClient.BanReason.系統鎖定, true, false, false,
                                    false);
                        }
                    }, 10 * 1000);
                }
    ...
    public static MapleProcess checkWGProcess(List<MapleProcess> processList) {
        ...
        for (MapleProcess process : processList) {
            if (WG_Property.containsValue(process.getMD5().toLowerCase())
                    || WG_Property.containsKey(process.getName())) {
                return process;
            }
        }
        return null;
