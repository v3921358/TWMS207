...
public class PackageHandler {
    ...
    /*
     * 19 = Successful 18 = One-of-a-kind Item is already in Reciever's delivery 17
     * = The Character is unable to recieve the parcel 15 = Same account 14 = Name
     * does not exist
     */
    public static void handleAction(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (operation) {
            case 1: { // Start Donald, 13 digit AS
                ...
                if (conv == 2) { // Donald
                    c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 10, loadItems(c.getPlayer())));
                }
                break;
            }
            case 3: { // Send Item
                    ...
                    if (accid != -1) {
                        if (accid != c.getAccID()) {
                            ...
                            if (inventId > 0) {
                                ...
                                if (item == null) {
                                    c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 17, null)); // Unsuccessfull
                                    return;
                                }
                                ...
                                if (c.getPlayer().getItemQuantity(item.getItemId(), false) >= amount) {
                                    ...
                                    if (!ii.isDropRestricted(item.getItemId()) && !ii.isAccountShared(item.getItemId())) {
                                        if (addItemToDB(item, amount, mesos, c.getPlayer().getName(), accid, recipientOn)) {
                                            ...
                                            c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 19, null)); // Successfull
                                        } else {
                                            c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 17, null)); // Unsuccessful
                                        }
                                    } else {
                                        c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 17, null)); // Unsuccessfull
                                    }
                            ...
                            // rClient.getSession().writeAndFlush(CField.sendPackageMSG(Actions.PACKAGE_MSG.getCode()));
                            // }
                        } else {
                            c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 15, null)); // Same acc error
                        }
                    } else {
                        c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 14, null)); // Name does not exist
                    }
                } else {
                    c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 12, null)); // Not enough mesos
                }
                break;
            }
            case 5: { // Recieve Package
                ...
                if (dp.getItem() != null && !MapleInventoryManipulator.checkSpace(c, dp.getItem().getItemId(),
                    ...
                    c.getSession().writeAndFlush(CField.sendPackageMSG((byte) 16, null)); // Not enough Space
                    return;
                ...
                c.getSession().writeAndFlush(CField.removeFromPackageList(false, packageid));
                break;
            }
            case 6: { // Remove package
                ...
                c.getSession().writeAndFlush(CField.removeFromPackageList(true, packageid));
                break;
            }
