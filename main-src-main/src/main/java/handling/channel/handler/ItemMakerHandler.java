...
public class ItemMakerHandler {
    ...
    public static enum CraftRanking {
        ...
        SOSO(21, 30), GOOD(22, 40), COOL(23, 50), FAIL(25, 20);
            ...
            case 1: { // Gem
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    MapleInventoryManipulator.addById(c, randGemGiven, (byte) (taken == randGemGiven ? 9 : 1),
                            "Made by Gem " + toCreate + " on " + FileoutputUtil.CurrentReadable_Date()); // Gem is always 1

                    c.getSession().writeAndFlush(EffectPacket.ItemMaker_Success());
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), EffectPacket.ItemMaker_Success(c.getPlayer()),
                            false);
                    ...
                    if (GameConstants.getInventoryType(toCreate) == MapleInventoryType.EQUIP) {
                        ...
                        MapleInventoryManipulator.addById(c, toCreate, (byte) 1,
                                "Made by Gem " + toCreate + " on " + FileoutputUtil.CurrentReadable_Date()); // Gem is
                        // always 1
                    }
                    ...
                    if (!stimulator || Randomizer.nextInt(10) != 0) {
                        ...
                        c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
                                EffectPacket.ItemMaker_Success(c.getPlayer()), false);
                    } else {
    ...
    private static void addEnchantStats(final Map<String, Integer> stats, final Equip item) {
        ...
        if (s != null && s != 0) {
            item.setWatk((short) (item.getWatk(0) + s));
        }
        s = stats.get("MAD");
        if (s != null && s != 0) {
            item.setMatk((short) (item.getMatk(0) + s));
        }
        s = stats.get("Speed");
        if (s != null && s != 0) {
            item.setSpeed((short) (item.getSpeed(0) + s));
        }
        s = stats.get("Jump");
        if (s != null && s != 0) {
            item.setJump((short) (item.getJump(0) + s));
        }
        s = stats.get("MaxHP");
        if (s != null && s != 0) {
            item.setHp((short) (item.getHp(0) + s));
        }
        s = stats.get("MaxMP");
        if (s != null && s != 0) {
            item.setMp((short) (item.getMp(0) + s));
        }
        s = stats.get("STR");
        if (s != null && s != 0) {
            item.setStr((short) (item.getStr(0) + s));
        }
        s = stats.get("DEX");
        if (s != null && s != 0) {
            item.setDex((short) (item.getDex(0) + s));
        }
        s = stats.get("INT");
        if (s != null && s != 0) {
            item.setInt((short) (item.getInt(0) + s));
        }
        s = stats.get("LUK");
        if (s != null && s != 0) {
            item.setLuk((short) (item.getLuk(0) + s));
        }
        s = stats.get("randOption");
        if (s != null && s != 0) {
            final int ma = item.getMatk(0), wa = item.getWatk(0);
            if (wa > 0) {
        ...
        if (s != null && s != 0) {
            final int str = item.getStr(0), dex = item.getDex(0), luk = item.getLuk(0), int_ = item.getInt(0);
            if (str > 0) {
    ...
    public static final void UseBag(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        c.getPlayer().dropMessage(5, "暫時無法使用擴展背包。");
        c.getSession().writeAndFlush(CWvsContext.enableActions());
        ...
        // toUse.setFlag(flag);
        // c.getSession().writeAndFlush(InventoryPacket.updateSpecialItemUse(toUse,
        // (byte) 4, toUse.getPosition(), true, chr));
        // }
        // c.getSession().writeAndFlush(CField.openBag(chr.getExtendedSlots().indexOf(itemId),
        // itemId, firstTime));
        // c.getSession().writeAndFlush(CWvsContext.enableActions());
    }
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c,
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            c.getSession().writeAndFlush(CField.onGatherRequestResult(reactor.getObjectId(), 9));
        } else {
            marr.setCustomData(String.valueOf(System.currentTimeMillis()));
            c.getSession().writeAndFlush(CField.onGatherRequestResult(reactor.getObjectId(), 12)); // ok to harvest,
            // gogo
            c.getPlayer().getMap().broadcastMessage(chr,
                    CField.onGatherActionSet(chr.getId(), item == null ? 0 : item.getItemId()), false);
            c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
        }
    }
    ...
    public static final void StopHarvest(final LittleEndianAccessor slea, final MapleClient c,
         ...
         * c.getPlayer().getMap().getReactorByOid(slea.readInt()); if (reactor == null
         * || !reactor.isAlive() || reactor.getReactorId() > 200011 ||
         * chr.getStats().harvestingTool <= 0 ||
         * reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 40000.0 ||
         * reactor.getState() < 3 || c.getPlayer().getFatigue() >= 100) { //bug in
         * global, so we use this to bug fix return; } Item item =
         * c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short)
         * c.getPlayer().getStats().harvestingTool); if (item == null || ((Equip)
    ...
    public static final void CraftEffect(final LittleEndianAccessor slea, final MapleClient c,
        ...
        if (profession != null && (c.getPlayer().getProfessionLevel(profession) > 0
                || (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
            int time = slea.readInt();
            if (time > 6000 || time < 3000) {
                time = 4000;
            }
            c.getSession().writeAndFlush(EffectPacket.showCraftingEffect(effect, (byte) chr.getDirection(), time,
                    effect.endsWith("Extract") ? 1 : 0));
            chr.getMap().broadcastMessage(chr, EffectPacket.showCraftingEffect(chr, effect, (byte) chr.getDirection(),
                    time, effect.endsWith("Extract") ? 1 : 0), false);
        }
    }
    ...
    public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (craftID) {
            case 92049000: { // disassembling
                ...
                quantity = (short) Randomizer.rand(3,
                        ItemConstants.類型.武器(itemId) || ItemConstants.類型.雙刀(itemId) || ItemConstants.類型.套服(itemId) ? 11 : 7);
                if (reqLevel <= 60) {
                    toGet = 4021013;
                } else if (reqLevel <= 90) {
                    toGet = 4021014;
            ...
            case 92049001: { // fusing.
                ...
                newEquip.setFlag(ItemFlag.CRAFTED.getValue());
                        ...
                        for (Triple<Integer, Integer, Integer> i : ce.targetItems) {
                            if (Randomizer.nextInt(100) < i.right) {
                                ...
                                if (GameConstants.getInventoryType(toGet) == MapleInventoryType.EQUIP) {
                                    ...
                                    if (Randomizer.isSuccess(theLevl * 2)) {
                                        first = ii.randomizeStats(first);
                                        cr = CraftRanking.COOL;
                                    }
                                    if (Randomizer.isSuccess(theLevl * (first.getUpgradeSlots() > 0 ? 2 : 1))) {
                                        first.resetPotential(false);
                                        cr = CraftRanking.COOL;
                                    }
                                    receive = first;
                                    receive.setFlag(ItemFlag.CRAFTED.getValue());
                                } else {
                                    receive = new Item(toGet, (short) 0, quantity);
                                }
                                if (ce.period > 0) {
                                    receive.setExpiration(System.currentTimeMillis() + (ce.period * 60000)); // period is in
                                    // minutes
                                }
                                receive.setOwner(chr.getName());
                                receive.setGMLog("裝備製作 " + craftID + " 時間:" + FileoutputUtil.CurrentReadable_Date());
        ...
        if (expGain > 0 && theLevl < 10) {
            ...
            if (Randomizer.isSuccess(chr.getTrait(MapleTraitType.craft).getLevel() / 5)) {
                expGain *= 2;
            }
            String s = "煉金術";
    ...
    public static final void ClearPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getSession().writeAndFlush(
                CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
    ...
    public static final void CurePot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getPlayer().getImps()[index].setState(1);
        c.getSession().writeAndFlush(
                CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.STATE.getValue(), index, false));
    ...
    public static final void RewardPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (itemid <= 0 || !MapleInventoryManipulator.checkSpace(c, itemid, (short) 1, "")) {
            c.getPlayer().dropMessage(1, "您的道具欄位不足。");
            c.getSession().writeAndFlush(CWvsContext.enableActions());
            return;
        }
