...
public class PlayerStorage {
    ...
    public PlayerStorage(int channel) {
        ...
        // Prune once every 5 seconds
        PingTimer.getInstance().register(new PersistingTask(), 5000);
    }
    ...
    public final int pendingCharacterSize(int world) {
        ...
        for (MapleCharacter chr : PendingCharacter.values()) {
    ...
    public class PersistingTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                final Iterator<Map.Entry<Integer, MapleCharacter>> itr = PendingCharacter.entrySet().iterator();

                while (itr.hasNext()) {
                    if (currenttime - StringTool
                            .parseLong(itr.next().getValue().getOneTempValue("Transfer", "TranferTime")) > 5000) { // 5
                        // sec
                        itr.next().getValue().updateOneTempValue("Transfer", "TranferTime", null);
                        itr.remove();
                    }
                }
            } finally {
